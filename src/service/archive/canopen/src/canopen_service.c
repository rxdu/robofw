...
bool StartCanopenService(CanopenServiceDef *def) {
  // create and start thread
  def->tconf.tid =
      k_thread_create(&def->tconf.thread,
                      def->tconf.stack,
                      K_THREAD_STACK_SIZEOF(def->tconf.stack),
                      CanopenServiceLoop, def, NULL, NULL,
...
void CanopenServiceLoop(void *p1, void *p2, void *p3) {
  CanopenServiceDef *def = (CanopenServiceDef *) p1;
  ...
  can.dev = def->sconf.dd_can->device;
  if (!can.dev) {
  ...
  while (reset != CO_RESET_APP) {
    ...
    printk("CAN %s, bitrate: %dk, filter: %d, node id: %d\n", def->sconf.dd_can->device->name,
           def->sconf.dd_can->bitrate / 1000, CONFIG_CAN_MAX_FILTER, CONFIG_CANOPEN_NODE_ID);

    err = CO_init(&can, CONFIG_CANOPEN_NODE_ID, def->sconf.dd_can->bitrate / 1000);
    if (err != CO_ERROR_NO) {
    ...
    printk("CANopen stack initialized\n");
