...
#define initialize_can(x, node_label, desc, msgqx)                                                 \
	{                                                                                          \
		desc.descriptor[x].device = device_get_binding(DT_LABEL(DT_ALIAS(node_label)));    \
...
bool InitCan()
{
	...
	for (int i = 0; i < DD_CAN_NUM; ++i)
		can_desc.descriptor[i].active = false;
...
int SendCanFrame(CanDescriptor *dd, uint32_t id, bool is_std_id, uint8_t data[], uint32_t dlc)
{
	if (!dd->active) {
		printk("[xCAN] Device inactive\n");
		return false;
